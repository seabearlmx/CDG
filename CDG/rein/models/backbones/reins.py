...
class Reins(nn.Module):
    def __init__(
        ...
        self.create_model()
        self.Proto = prototype_dist_estimator(feature_num=self.embed_dims, layer_num=self.num_layers)
    ...
    def return_auto(self, feats):
        if self.link_token_to_query:
            ...
            return feats, querys, self.learnable_tokens, self.Proto
        else:
            return feats
    ...
    def forward(
        self, feats: Tensor, layer: int, B: int, H: int, W: int, labels=None, batch_first=False, has_cls_token=True
    ) -> Tensor:
        if labels is not None:
            pro_feat = feats[:, 1:, :].permute(0, 2, 1).reshape(B, -1, H, W).contiguous()
            Bh, Ch, Hh, Wh = pro_feat.size()
            src_mask = F.interpolate(labels.unsqueeze(0).float(), size=(Hh, Wh), mode='nearest').squeeze(
                0).long()
            src_mask = src_mask.contiguous().view(Bh * Hh * Wh, )
            pro_feat = pro_feat.permute(0, 2, 3, 1).contiguous().view(Bh * Hh * Wh, Ch)
            Proto_srouce = self.Proto.forward(features=pro_feat.detach(), labels=src_mask, layer_i=layer)
        if batch_first:
...
class LoRAReins(Reins):
    ...
    def return_auto(self, feats):
        if self.link_token_to_query:
            ...
            querys = self.merge(tokens.flatten(-2, -1))
            return feats, querys, self.learnable_tokens_a @ self.learnable_tokens_b, self.Proto
        else:
            return feats
